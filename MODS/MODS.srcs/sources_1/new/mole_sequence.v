...
module mole_sequence(input is_master, input input_mole_state0, input_mole_state1, input_mole_state2, input [31:0] input_mole_y0, input_mole_y1, input_mole_y2, input basys_clock, clk_25MHz, reset, left_click, btnC, btnU, btnL, btnR, btnD, input [11:0] xpos, ypos, input [12:0] pixel_index, input [31:0] switch_points, output reg [15:0] oled_data, output [31:0] total_points, output reg bomb_defused = 1, output mole_state0, mole_state1, mole_state2, output [31:0] mole_y0, mole_y1, mole_y2);
    ...
    reg [2:0] mole_state, mole_hit;
    reg wrong_hit = 0;
    wire [31:0] generated_mole_count [2:0], generated_mole_position_change[2:0];
    //mole_position_change[2:0]
    reg [31:0] prev_mole_count [2:0], mole_position_change[2:0], mole_y[2:0], mole_count [2:0], mole_dead_count [2:0], wrong_hit_count = 0, count = 0;
    wire moleC, moleR, moleL, mouse_arrow, click_mole[2:0];
    
    assign moleC = (((x - 48) ** 2 + (y - mole_y[1]) ** 2) <= 64);
    assign moleL = (((x - 24) ** 2 + (y - mole_y[0]) ** 2) <= 81) || (y > mole_y[0] && y <= mole_y[0] + 9 && x <= 24 + 9 && x >= 24 - 9);
    assign moleR = (((x - 72) ** 2 + (y - mole_y[2]) ** 2) <= 81) || (y > mole_y[2] && y <= mole_y[2] + 9 && x <= 72 + 9 && x >= 72 - 9);
    assign mouse_arrow = (((x - xpos) ** 2 + (y - ypos) ** 2) < 5);
    
    assign click_mole[0] = debounced_left_click && (((mole_y[0] - ypos) ** 2 + (24 - xpos) ** 2 <= 81) || (ypos > mole_y[0] && ypos <= mole_y[0] + 9 && xpos <= 24 + 9 && xpos >= 24 - 9));
    assign click_mole[1] = debounced_left_click && ((mole_y[1] - ypos) ** 2 + (48 - xpos) ** 2 <= 64);
    assign click_mole[2] = debounced_left_click && (((mole_y[2] - ypos) ** 2 + (72 - xpos) ** 2 <= 81) || (ypos > mole_y[2] && ypos <= mole_y[2] + 9 && xpos <= 72 + 9 && xpos >= 72 - 9));
    ...
    //defuse sequence:
    reg [31:0] defuse_timer = 0, prev_defuse_length = 1, actual_defuse_length = 8;
    reg launch = 0;
    ...
    assign total_points = switch_points + points;
    ...
    begin
        ...
        if (reset) begin
            ...
            mole_count[2] = 0;
            prev_mole_count[0] = 1;
            prev_mole_count[32] = 1;
            prev_mole_count[64] = 1;
        ...
        mole_y[1] <= (is_master) ? (32 + 8 - mole_position_change[1] % 16) : input_mole_y1;
        mole_y[0] <= (is_master) ? (32 + 7 - mole_position_change[0] % 16) : input_mole_y0;
        mole_y[2] <= (is_master) ? (32 + 9 - mole_position_change[2] % 16) : input_mole_y2;
 
        
        //displaying (arrow, moles, points)
        if (launch) begin
            oled_data <= defuse_oled_data;
        end 
        ...
        wrong_hit = (wrong_hit_count > 2) ? 0 : wrong_hit;
        ...
        defuse_timer <= (count == 0) ? ((defuse_timer > 302) ? 0 : defuse_timer + 1) : defuse_timer;
        if (defuse_timer == 300) begin
            launch = 1;
            bomb_defused = 0;
            actual_defuse_length = 4 + (prev_defuse_length % 6);
        end
        if (launch && correct_press_count == actual_defuse_length) begin
            launch = 0;
            bomb_defused = 1;
        end
    end
